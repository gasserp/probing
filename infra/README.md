...
az vm run-command invoke \
  ...
  --scripts 'set -eu; cloud-init status --wait --long; cd /opt/probing; running="$(docker compose -f deploy/docker-compose.yml ps --services --status running)"; for service in collector cowrie nginx; do printf "%s\n" "$running" | grep -qx "$service"; done; docker compose -f deploy/docker-compose.yml ps'
```
