...
func TestReconcileReceiptsCleansUpOrphanedReceipt(t *testing.T) {
	root := t.TempDir()
	outbox := filepath.Join(root, "outbox")
	if err := os.Mkdir(outbox, 0o750); err != nil {
		t.Fatal(err)
	...
	// Write the receipt file, then remove the envelope file, to simulate a
	// crash between envelope removal and receipt removal in acknowledge()
	receiptPath := filepath.Join(outbox, publication.ReceiptFileName(envelope.FileName))
	if err := os.WriteFile(receiptPath, receiptBytes, 0o640); err != nil {
		t.Fatal(err)
	}
	if err := os.Remove(envelopePath); err != nil {
		t.Fatal(err)
	}
	// Mark the batch as published (as would happen before any removals)
	receiptHash := sha256.Sum256(receiptBytes)
	if err := state.MarkBatchPublished(
		context.Background(),
		envelope.Batch.Payload.Sequence,
	...
	// This should not error, even though the envelope file is missing
	if err := publisher.reconcileReceipts(context.Background()); err != nil {
		t.Fatal(err)
	}
	// The orphaned receipt should have been cleaned up
	entries, err = os.ReadDir(outbox)
	if err != nil || len(entries) != 0 {
		t.Fatalf("outbox after reconciliation with orphaned receipt = %v, %v", entries, err)
	}
}
